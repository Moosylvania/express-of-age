...
var validateBirthDate = function(age, birthdate){
...
var validateBot = function(useragent){
    if(useragent) {
    ...
    return function ofAge(req, res, next){
        ...
        if(req.method == "POST" && req.path == url){
            if(req.body.birthdate){
                if(validateBirthDate(age, req.body.birthdate)){
        ...
        if(req.cookies && req.cookies.ofage && req.path != url){
            return next();
        } else if (req.cookie && req.cookies.ofage && req.path == url) {
            return res.redirect('/');
        } else if( req.path != url ){
            if(validateBot(req.headers['user-agent']))
            {
                req.cookies.ofage = true;
                res.cookie('ofage', true, { maxAge: 900000, path:'/' });
                return next();
